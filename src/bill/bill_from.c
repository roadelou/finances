...
Error bill_from_ptr(Bill *bill, const void *src) {
  ...
  // Finally we copy the string to its allocated buffer (once again, kind of a hack)
...
Error bill_from_file(Bill *bill, FILE *src) {
  ...
  if (read_beginning != beginning) {
    return error(-1, "Expected to read %ld bytes from file for a bill, but only read %ld instead", beginning, read_beginning);
  }
  // else...
  ...
  if (read_reason != bill->len) {
    return error(-2, "Expected to read %d bytes for reason of bill %d, but only read %ld instead", bill->len, bill->number, read_reason);
  }
  // else...
