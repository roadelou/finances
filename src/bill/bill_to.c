...
Error bill_to_ptr(const Bill *bill, void *dest, const int len) {
  ...
  if (size + 1 > len) {
    return error(-1,
                 "The size allocated to serialize bill %d was %d bytes, but it "
                 "needs %ld",
                 bill->number, len, size);
  }
  // else...
  ...
  /*
  What follows is kind of a hack. Because the only difference between the
  serialized Bill and the one in memory is that reason contains the whole string
  and not just a pointer to it, we copy the content of the string to the
  location where its pointer would normally be.
  */
  strncpy((char *)&(dest_as_bill->reason), bill->reason,
          bill->len + 1); // +1 for null termination
  return ok();
}
...
Error bill_to_file(const Bill *bill, FILE *dest) {
  ...
  if (written_beginning != beginning) {
    return error(-1,
                 "Expected to write %ld bytes to file for bill %d, but only "
                 "wrote %ld instead",
                 beginning, bill->number, written_beginning);
  }
  // else...
  ...
  if (written_reason != bill->len + 1) {
    return error(-2,
                 "Expected to write %d bytes for reason of bill %d, but only "
                 "wrote %ld instead",
                 bill->len + 1, bill->number, written_reason);
  }
  // else...
